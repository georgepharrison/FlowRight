...
public sealed class NegativeRule<T> : IRule<T>
{
    ...
    /// <returns>An error message if the value is not negative; otherwise, null.</returns>
    public string? Validate(T value, string displayName)
    {
        ...
        if (zero is null)
        {
            return $"{displayName} must be negative.";
        }

        return Comparer<T>.Default.Compare(value, (T)zero) < 0
            ? null
            : $"{displayName} must be negative.";
    }
